...
public class GrapplingHook : MonoBehaviour {
	...
	[SerializeField] private float step = .5f; // шаг сокращения длинны
	...
	{
		hit = Physics2D.Linecast(transform.position, position, layerMask); //Отрезок линии отбрасываемый на коллайдеры  
		//(начальн. точка, конеч. точка, фильтрдля определеных слоев)
	...
	{
		return Vector3.Cross(hit.point - body.position, Vector3.forward).normalized; //Векторное произведение 2 векторов
	}
